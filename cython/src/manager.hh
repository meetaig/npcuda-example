class GPUAdder
{
    int* array_device;      // pointer to the GPU memory where the array is stored
    int* array_host;        // pointer to the CPU memory where the array is stored
...
public:
   ...
   */
    ...
    void retreive();                                 /*   gets results back from GPU, putting them
                                                        in the memory that was passed in the
                                                        constructor */
    void retreive_to (int* INPLACE_ARRAY1, int DIM1);/* gets results back from the gpu, putting
                                                        them in the supplied memory location */
};
